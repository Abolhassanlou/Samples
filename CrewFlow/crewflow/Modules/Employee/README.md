...
- `WorkerController::update()` now allows a worker to edit their own personal facts (name, DOB, address, etc. — matching the worker portal's "My info → Personal details" section, meant to be self-filled) — including their own `work_authorization_type`/`work_authorization_expiry_date` **claim** (e.g. "I have an Austrian passport", "my permit expires on X"). But `status` and `work_authorization_status` are still stripped out unless the requester has `users.manage`, so a worker can never self-approve their own work authorization or activate their own account — an admin still has to look at the uploaded passport/permit document and set `work_authorization_status` themselves; `WorkerEligibility` gates on that status alone, never on the type/expiry claim.
...
Two places use the expiry date once it's on file:
- **`WorkerEligibility::isAssignable()`** (Shift module) — refuses to assign a worker whose expiry date has already passed, *independent of* what `work_authorization_status` currently says. This is a safety net: an admin can set status to `valid` today and simply forget to revisit it the day the document actually lapses. This blocks the admin's own "Assign" action on the Shifts page exactly the same way it blocks anyone else — `AssignmentController::store()` calls this same check, no separate admin-side bypass exists.
- **`GET /api/workers/expiring-documents`** — every worker whose expiry date is already past or due within 30 days, most urgent first. This is a plain pull the admin dashboard shows on every login, not a push — no email/notification is sent by this pass. `WorkerDirectoryController::index()`'s `?eligible=1` filter was also updated to match `WorkerEligibility` exactly (it previously didn't check expiry at all, only status).

**`workers:expire-authorizations`** (an Artisan command, `Console/Commands/ExpireWorkAuthorizations.php`) closes the remaining gap: the safety net above stops an *expired* worker from being assigned, but their `work_authorization_status` column itself would otherwise just sit at `valid` forever — invisible to the Workers page's own status filter, since that filter (see below) reads the stored column, not a live date comparison. This command flips `valid` to `expired` the moment the date has passed, across every tenant (`tenancy()->runForMultiple(null, ...)`), and is scheduled daily from `EmployeeServiceProvider::registerSchedule()` — registered there rather than the core app's `routes/console.php` so this module stays self-contained (installing/removing it is a single directory swap, no edit needed outside `Modules/Employee`). It deliberately writes `expired`, not `pending` — `pending` already means something different (never yet reviewed at all) and conflating the two would make either filter less useful. `Worker.status` (the *employment* status) is untouched — a lapsed document doesn't take someone off the roster, it just makes them unassignable until it's renewed and re-confirmed.

For this command to actually run on a schedule (not just when invoked manually), the server needs `* * * * * php artisan schedule:run` in its system crontab — standard for any Laravel app's scheduler, not specific to this command. Nothing in this pass sets that up; it's an infrastructure step outside the codebase, worth calling out because a working command with no schedule:run entry is silently a no-op.

`WorkerDirectoryController::index()` now also accepts `?work_authorization_status=`, so `expired` (or `pending`, `valid`, etc.) is filterable directly alongside every other search/filter this directory already supports (branch, qualification, availability, ...) — not only visible on the separate expiring-documents dashboard list. `WorkerDirectoryResource` now also exposes `work_authorization_type`/`work_authorization_expiry_date` on every row (previously only `work_authorization_status`), so a filtered list actually shows the date/type an admin filtered for, not just the status.

## Contract expiry — the same stale-status problem, one layer down

`EmploymentContract.status` has exactly the same gap `work_authorization_status` had: `isCurrentlyActive()` (used by `WorkerEligibility`) already checks the actual `end_date` against today, not just the stored `status` column, so assignment safety was never at risk from a contract sitting at `status: active` past its own end date. But that stale `active` label is what the admin's own Contracts table (on a worker's profile) shows, since it displays the raw column — a lapsed contract looks identical to a genuinely active one until someone actually tries and fails to assign that worker.
