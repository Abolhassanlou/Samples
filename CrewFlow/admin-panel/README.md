...
src/
  ...
  views/DashboardView.vue "Work authorization needs attention" — every worker whose passport/visa/ID expiry is already past or due within 30 days (users.manage only), each row linking straight to that worker's profile
  views/WorkersView.vue   dispatcher-facing worker search (qualification/branch/contract/availability/work-authorization-status filters) — a card shows an amber "Employment: pending" badge and/or a red "Work authorization: expired" badge, each explicitly labeled and colored differently, since a bare "pending" with no label was ambiguous about which status it meant
  ...
  views/ChatView.vue          two-column layout — a conversation list on the left ("+ New broadcast" above it), and on the right either an empty state, a thread view (reply to any existing conversation), or the broadcast composer. The recipient checklist there is filterable — branch/qualification/work-time-model/night-shift/eligible-now, the same filter set WorkersView offers, reused via the same fetchWorkers(filters) call rather than duplicated client-side; changing a filter re-queries and drops any selected worker no longer in the narrowed results. Sends as separate private threads, not a shared group — see the Chat module's README for why. Direct/group conversations aren't started here, only replied to — the Chat module leaves that to whichever flow first creates them (a worker messaging in, or the automatic per-Event chat once that's built)
  api/chats.js                 fetchConversations()/fetchMessages()/sendMessage()/startDirectConversation()/sendBroadcast()
